                  ...
                  {units.map((u,i)=> (
                    <div key={u.category} className="border border-border/40 rounded-md p-3">
                      ...
                      <div className="mt-3 grid grid-cols-1 md:grid-cols-4 gap-2">
                        <input type="number" min="0" step="0.1" placeholder="Meters" value={u.meters} onChange={(e)=>setUnits(prev=>prev.map((x,idx)=>idx===i?{...x,meters:e.target.value}:x))} className="p-2 border border-border/40 rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20" />
